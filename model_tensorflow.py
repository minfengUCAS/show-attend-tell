...
class Caption_Generator():
    ...
    def build_model(self):
        context = tf.placeholder("float32", [self.batch_size, self.ctx_shape[0], self.ctx_shape[1]]) # [batch_size,196,512]
        sentence = tf.placeholder("int32", [self.batch_size, self.n_lstm_steps]) # [batch_size, n_lstem_steps]
        mask = tf.placeholder("float32", [self.batch_size, self.n_lstm_steps]) # [batch_size,n_lstem_steps]

        h, c = self.get_initial_lstm(tf.reduce_mean(context, 1))
        # h:[batch_size,1,256]    c:[batch_size,1,256]

        # TensorFlow가 dot(3D tensor, matrix) 계산을 못함;;; ㅅㅂ 삽질 ㄱㄱ
        context_flat = tf.reshape(context, [-1, self.dim_ctx]) # [batch_size*196,512]
        context_encode = tf.matmul(context_flat, self.image_att_W) # [batch_size*196,512]
        context_encode = tf.reshape(context_encode, [-1, ctx_shape[0], ctx_shape[1]]) # (batch_size,1, 196, 512)
        ...
        for ind in range(self.n_lstm_steps):
            ...
            if ind == 0:
                ...
                with tf.device("/cpu:0"):
                    word_emb = tf.nn.embedding_lookup(self.Wemb, sentence[:,ind-1]) # [batch_size,256]

            x_t = tf.matmul(word_emb, self.lstm_W) + self.lstm_b # (batch_size, 256*4)
            ...
            onehot_labels = tf.sparse_to_dense( concated, tf.pack([self.batch_size, self.n_words]), 1.0, 0.0) # [batch_size,n_word]

            context_encode = context_encode + \
                 tf.expand_dims(tf.matmul(h, self.hidden_att_W), 1) + \
                 self.pre_att_b # (batch_size,1, 196, 512)

            context_encode = tf.nn.tanh(context_encode) # (batch_size,1, 196, 512)
            ...
            alpha = tf.nn.softmax( alpha ) # [batch_size,196]

            weighted_context = tf.reduce_sum(context * tf.expand_dims(alpha, 2), 1) # [batch_size,196,512]

            lstm_preactive = tf.matmul(h, self.lstm_U) + x_t + tf.matmul(weighted_context, self.image_encode_W)
            # [batch_size,196,256*4]
            i, f, o, new_c = tf.split(1, 4, lstm_preactive) # [batch_size,196,256]
...
ctx_shape=[196,512]
pretrained_model_path = './model/model-112'
#############################
###### 잡다한 Parameters #####
...
def train(pretrained_model_path = pretrained_model_path): # 전에 학습하던게 있으면 초기값 설정.
    ...
    for epoch in range(113, n_epochs, 1):
        for start, end in zip( \
            ...
            print "Current Cost: ", loss_value, "Epoch: ", epoch, "Sample:", start
        saver.save(sess, os.path.join(model_path, 'model'), global_step=epoch)

def test(test_feat='./guitar_player.npy', model_path='./model/model-128', maxlen=30):
    annotation_data = pd.read_pickle(annotation_path)
    captions = annotation_data['caption'].values
    wordtoix, ixtoword, bias_init_vector = preProBuildWordVocab(captions)
    n_words = len(wordtoix)
...
#    ipdb.set_trace()
# train()
# for i in xrange(65, 112):
#     test(model_path='./model/model-'+str(i))
# train()
test()
