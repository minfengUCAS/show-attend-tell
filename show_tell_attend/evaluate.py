...
tf.flags.DEFINE_integer("min_global_step", 1,
...
def evaluate_model(sess, model, global_step, summary_writer, summary_op):
  ...
  for i in xrange(num_eval_batches):
    cross_entropy_losses = sess.run([
        model.target_cross_entropy_losses
    ])
    sum_losses += np.sum(cross_entropy_losses)
    if not i % 100:
...
def run():
  ...
  with g.as_default():
    ...
    model = show_attend_tell_model.ShowAttendTellModel(model_config, mode="eval")
    model.build()
